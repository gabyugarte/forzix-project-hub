...
const searchSchema = z.object({
  categoria: z
    .enum(["todos", "bisagras", "correderas", "pistones", "push-open", "tiradores", "accesorios"])
    .optional(),
});
...
function ProductosPage() {
  ...
  const [category, setCategory] = useState<FilterValue>(categoria ?? "todos");

  const filtered = useMemo(() => {
    const term = query.trim().toLowerCase();
    return products.filter((product) => {
      const matchCategory = category === "todos" || product.category === category;
      const matchQuery = term === "" || product.name.toLowerCase().includes(term);
      ...
      <section className="border-b border-border bg-surface">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          ...
          <p className="mt-4 max-w-2xl text-base text-muted-foreground sm:text-lg">
            Herrajes y accesorios para la fabricación y montaje de muebles. Encuentra lo que
            necesitas y consúltanos precio y disponibilidad por WhatsApp.
          </p>
        </div>
      </section>
